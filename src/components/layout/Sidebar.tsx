  ...
  return (
    <>
      ...
      <aside
        ...
        `}
        style={{
          width: effectiveCollapsed ? 60 : 280,
          background: '#1A1F2B',
      ...
      >
        {effectiveCollapsed ? (
          ...
          /* ---- Expanded sidebar (mobile + desktop) ---- */
          <>
            {/* Header */}
            <div
              className="flex items-center justify-between shrink-0"
              style={{ height: 64, padding: '0 12px 0 20px', borderBottom: '0.8px solid rgba(255, 255, 255, 0.06)' }}
            >
              <button onClick={handleNewChat} className="flex items-center hover:opacity-80 transition-opacity">
                <AudiLogo size={60} className="text-white" />
              </button>
              <button
                onClick={() => (isMobile ? closeMobile() : setCollapsed(true))}
                className="flex items-center justify-center transition-all hover:bg-white/10 active:scale-95"
            ...
            <div style={{ padding: '16px 14px 0' }}>
              <button
                ...
                style={{ height: 44, borderRadius: 10, background: '#657081', gap: 8, marginBottom: 12 }}
              >
                <Plus size={16} color="white" />
                <span style={{ color: 'white', fontSize: 14, fontWeight: 600, lineHeight: '21px' }}>New Chat</span>
              </button>
            ...
            {/* Chat History */}
            <div className="flex-1 overflow-y-auto flex flex-col min-h-0" style={{ padding: '12px 10px 0' }}>
              <div
                className="shrink-0"
                style={{ color: '#6A7282', fontSize: 11, fontWeight: 600, letterSpacing: 0.5, lineHeight: '16px', marginBottom: 8, paddingLeft: 6 }}
              >
                RECENT
              </div>
              ...
              <div className="flex flex-col" style={{ gap: 2 }}>
                {filteredChats.map((chat) => (
                  <button
                    ...
                    style={{ padding: '10px 10px', gap: 12 }}
                  >
                    <img
                      src={chat.thumbnail}
                      alt={chat.carName}
                    ...
                    <div className="flex-1 flex flex-col overflow-hidden" style={{ gap: 2 }}>
                      <span className="truncate" style={{ color: '#FAFAFA', fontSize: 13, fontWeight: 600, lineHeight: '20px' }}>
                        {chat.carName}
                      </span>
                      <span className="truncate" style={{ color: '#6A7282', fontSize: 12, lineHeight: '18px' }}>
                        {chat.promptPreview}
                      </span>
                    </div>
                  </button>
            ...
            {/* Footer */}
            <div className="shrink-0" style={{ padding: '12px 14px 16px', borderTop: '0.8px solid rgba(255, 255, 255, 0.06)' }}>
              <button
                className="w-full flex items-center justify-center transition-all hover:bg-white/[0.06]"
                style={{ height: 36, borderRadius: 10 }}
              >
                <span style={{ color: '#6A7282', fontSize: 13, lineHeight: '20px' }}>Clear History</span>
              </button>
            </div>
          </>
        )}
