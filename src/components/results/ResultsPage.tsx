    ...
    <div className="flex-1 flex flex-col" style={{ gap: 12 }}>
      ...
      >
        ...
        <button
          ...
          style={{ top: '50%', left: '50%', transform: 'translate(-50%, -50%)', width: 56, height: 56, borderRadius: 9999, background: 'rgba(255, 255, 255, 0.10)' }}
        >
          {playing ? <Pause size={24} color="white" fill="white" /> : <Play size={24} color="white" fill="white" />}
        </button>
        <div className="absolute inset-x-0 bottom-0" style={{ padding: '16px' }}>
        ...
        <div className="flex flex-col" style={{ gap: 24 }}>
          ...
          <div>
            <div style={{ marginBottom: 12 }}>
              <span style={{ color: '#99A1AF', fontSize: 12, fontWeight: 600, letterSpacing: 0.6, lineHeight: '12px' }}>ASPECT RATIO</span>
            </div>
          ...
          <div>
            <div style={{ marginBottom: 12 }}>
              <span style={{ color: '#99A1AF', fontSize: 12, fontWeight: 600, letterSpacing: 0.6, lineHeight: '12px' }}>RESOLUTION</span>
            </div>
          ...
          <div>
            <div style={{ marginBottom: 12 }}>
              <span style={{ color: '#99A1AF', fontSize: 12, fontWeight: 700, letterSpacing: 0.6, lineHeight: '12px' }}>FRAME RATE</span>
            </div>
            <div className="flex" style={{ gap: 8 }}>
          ...
          <div>
            <div style={{ marginBottom: 12 }}>
              <span style={{ color: '#99A1AF', fontSize: 12, fontWeight: 600, letterSpacing: 0.6, lineHeight: '12px' }}>COLOR GRADING (LUT)</span>
            </div>
                ...
                <>
                  ...
                  <div className="absolute left-0 right-0 z-50" style={{ top: 40, background: '#1a1d24', borderRadius: 8, border: '0.8px solid rgba(255, 255, 255, 0.10)', overflow: 'hidden' }}>
                    {['None', 'Cinematic', 'Moody', 'Warm Vintage', 'Cool Teal', 'Desaturated'].map((opt) => (
                      <button key={opt} onClick={() => { setColorGrade(opt); setColorOpen(false) }} className="w-full text-left transition-colors hover:bg-white/5" style={{ padding: '8px 12px', color: opt === colorGrade ? 'white' : '#99A1AF', fontSize: 14, background: opt === colorGrade ? 'rgba(255,255,255,0.05)' : 'transparent' }}>
                        {opt}
                      </button>
                    ))}
                  </div>
          ...
          {!done ? (
              ...
              <div className="w-full">
                ...
                <div className="flex items-center justify-between" style={{ marginTop: 8 }}>
                  <span style={{ color: '#6A7282', fontSize: 12, lineHeight: '18px' }}>
                    {exportProgress < 40 ? 'Encoding video...' : exportProgress < 80 ? 'Applying color grade...' : 'Writing file...'}
                  </span>
            ...
            <div className="flex flex-col items-center" style={{ gap: 20 }}>
              ...
              <div className="flex flex-col items-center" style={{ gap: 4 }}>
                <span style={{ color: 'white', fontSize: 16, fontWeight: 600, lineHeight: '24px' }}>
                  Your video has been exported!
                </span>
                <span style={{ color: '#6A7282', fontSize: 14, lineHeight: '21px' }}>
                  4K &bull; 30fps &bull; 9.0s &bull; ~45 MB
                </span>
              </div>
              ...
              <div className="w-full">
                ...
                <div className="flex" style={{ gap: 8 }}>
                  {socials.map(({ name, icon: Icon }) => (
                    <button
                      key={name}
                      className="flex-1 flex flex-col items-center justify-center transition-all hover:brightness-125"
                      style={{
                        height: 72,
                        borderRadius: 12,
                        background: 'rgba(255, 255, 255, 0.05)',
              ...
              <div className="w-full flex flex-col" style={{ gap: 10 }}>
                ...
                <button
                  ...
                  className="w-full flex items-center justify-center transition-all hover:brightness-110"
                  style={{ minHeight: 48, borderRadius: 999, background: '#181D25', boxShadow: '0px 0px 0px 1px #2C343F inset', gap: 8 }}
                >
...
export default function ResultsPage() {
  ...
  return (
    <div className="flex flex-col w-screen h-screen overflow-hidden" style={{ background: '#101319' }}>
      ...
      {/* Content — centered */}
      <div className="flex-1 overflow-y-auto flex justify-center" style={{ padding: '32px 40px' }}>
        <div className="flex" style={{ gap: 32, maxWidth: 1360, width: '100%' }}>
          {/* Left: Videos + Stats */}
          <div className="flex-1 flex flex-col" style={{ gap: 24 }}>
            {/* Compare bar */}
            <div className="flex items-center" style={{ gap: 12 }}>
              <span style={{ color: '#99A1AF', fontSize: 14, fontWeight: 600, lineHeight: '21px' }}>Compare:</span>
              {versions.map((v) => {
                ...
                return (
                  <button
                    ...
                    style={{ height: 35, paddingLeft: 12, paddingRight: 12, borderRadius: 10, background: active ? 'rgba(10, 130, 223, 0.10)' : 'rgba(255, 255, 255, 0.05)', border: active ? '0.8px solid rgba(10, 130, 223, 0.50)' : '0.8px solid rgba(255, 255, 255, 0.10)', gap: 8 }}
                  >
            ...
            <div className="flex" style={{ gap: 16 }}>
              {[
                ...
                <div key={stat.label} className="flex-1 flex flex-col" style={{ padding: '16px', borderRadius: 14, background: 'rgba(255, 255, 255, 0.05)', border: '0.8px solid rgba(255, 255, 255, 0.05)', gap: 4 }}>
                  <span style={{ color: '#99A1AF', fontSize: 12, lineHeight: '18px' }}>{stat.label}</span>
                  <span style={{ color: stat.color, fontSize: 20, fontWeight: 600, lineHeight: '30px' }}>{stat.value}</span>
                </div>
              ))}
            </div>
          </div>
